...
from .mamba import Mamba2,NdMamba2
...
class SegVG(nn.Module):
    def __init__(self, args):
        ...
        self.mamba2_visual = NdMamba2(cin=400,cout=400,mamba_dim=64)
        # self.mamba2_text = Mamba2(d_model=768, n_layer=6, d_state=128, d_conv=4, expand=2, headdim=64, chunk_size=64)
        self.mamba2_text = NdMamba2(cin=20,cout=20,mamba_dim=768)
    ...
    def forward(self, img_data, text_data):
        ...
        # visual_src = pad_to_multiple(visual_src, 64, dim=0)
        # visual_mask = pad_to_multiple(visual_mask, 64, dim=1, value=1)
        # a = visual_src
        # print(f'a ；{a.shape}')
        # b = visual_src.permute(1, 0, 2)
        # print(f'b ；{b.shape}')
        # visual_src = self.mamba2_visual(visual_src.permute(1, 0, 2))[0].permute(1, 0, 2)
        visual_src = self.mamba2_visual(visual_src.permute(1, 0, 2)).permute(1, 0, 2)

        # language encode #
        text_src, extended_attention_mask, head_mask = self.textmodel(input_ids=text_data.tensors, attention_mask=text_data.mask)
        # c = text_src
        # print(f'c shape: {c.shape}')
        # text_src = pad_to_multiple(text_src, 64, dim=1)
        # # text_mask = pad_to_multiple(text_mask, 64, dim=1, value=1)
        # text_src = self.mamba2_text(text_src)[0]
        text_src = self.mamba2_text(text_src)
...
def pad_to_multiple(tensor, multiple, dim=1, value=0):
    ...
    if seq_length % multiple != 0:
        pad_length = multiple - (seq_length % multiple)
        padding = [0] * (2 * tensor.ndim)
        padding_index = 2 * (tensor.ndim - 1 - dim)
        padding[padding_index + 1] = pad_length  # pad_right for the specified dim
        tensor = F.pad(tensor, padding, value=value)
    return tensor
class MLP(nn.Module):
